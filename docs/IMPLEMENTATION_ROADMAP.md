...
- Keep shared browser state in one generated helper (`web/gsi-context.js`) so nested pages do not drift in their URL handling.
- Establish shared CSS tokens (`web/gsi-tokens.css`) before migrating page-specific rules, so the Marathon type, spacing, color, and geometry language has one governed source.
...
**Acceptance:** no entry-card overlap at the checked breakpoints; reloading a filtered Albums URL retains Albums; opening an album then a song can return to that same state.
...
`python build.py --site-only --validate-links`, and the homepage presentation and
state layers now live in `web/gsi-home.css` and `web/home-page.js`. The generated
...
lives in `web/artist-room.js` and `web/album-room.js`. Asset budgeting, broader
tests, and the final device/deployment sweep remain future work. The P53 landing
query-state and focus helper now lives in `web/p53-landing.js` alongside the
other browser helpers. Permanent transmission routing and sharing now live in
`web/p53-transmission.js` with a generated context node.
