...
def build_entry_page(item: dict, artist_counts: dict[str, int] | None = None) -> None: #HTML review page
    ...
    artist_display = (
        f'<a class="artist-link" href="{artist_href}">{html.escape(item["artist"])}</a>'
        if artist_room_exists else html.escape(item["artist"])
    )
    if artist_room_exists:
        also_paths.append(f'<a href="{artist_href}">ARTIST PAGE : {html.escape(item["artist"])}</a>')
    if in_p53:
...
<head>
    ...
    <style>
        ...
        /* Context stays inside the masthead; the long-entry rail uses outer desktop space. */
        .entry-nav {{ display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:12px; }}
        .entry-nav .back {{ margin:0; }}
        .signal-trace {{
            display:flex;
            align-items:center;
            gap:7px;
        ...
        .entry-signal {{ text-align:right; }}
        .signal-trace a {{ color:color-mix(in srgb,var(--accent),white 38%); text-decoration:none; }}
        ...
        @media(max-width:760px) {{
            ...
            .entry-nav {{ grid-template-columns:minmax(0,1fr); gap:8px; }}
            .entry-nav .entry-signal {{ display:none; }}
            .signal-trace {{ grid-column:auto; width:100%; min-width:0; padding-top:8px; border-top:1px solid rgba(255,255,255,.09); overscroll-behavior-inline:contain; scrollbar-width:none; }}
...
<body>
    <main class="page">
        <header class="entry-nav">
            <a class="back" id="archive-return" href="../index.html"><span aria-hidden="true">←</span> GSI</a>
            <span class="entry-signal">SIGNAL / {html.escape(item["artist"])}</span>
            <nav class="signal-trace" aria-label="Archive path">
                <a id="trace-archive" href="../index.html">ARCHIVE</a>
                ...
                <span id="trace-artist-separator" class="trace-separator hidden">→</span>
                <span aria-current="page">{html.escape(item["track"])}</span>
            </nav>
        </header>
    ...
    <script>
        ...
        document.querySelectorAll("#archive-return, #trace-archive").forEach(link => link.href = archiveHref);
        ...
        if (hasArtistRoom && artistRoute === expectedArtistRoute) {{
            ...
            document.querySelector("#trace-artist-separator").classList.remove("hidden");
            const archiveReturn = document.querySelector("#archive-return");
            archiveReturn.href = artistHref;
            archiveReturn.replaceChildren();
            const arrow = document.createElement("span");
            arrow.setAttribute("aria-hidden", "true");
            arrow.textContent = "←";
            archiveReturn.append(arrow, ` ${{artistName}}`);
        }}
    </script>
</body>
</html>
...
<head>
    ...
    <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .p53-trace {{ display:flex; align-items:center; gap:7px; min-width:0; overflow:auto; color:rgba(255,255,255,.5); font-size:10px; font-weight:900; letter-spacing:.16em; white-space:nowrap; }}
        .p53-trace a {{ color:#ff8bc2; font-size:10px; }}
...
<body>
    <main class="p53-page">
        <header class="p53-nav">
            <a id="p53-archive-return" href="../index.html">← GSI</a>
            <nav class="p53-trace" aria-label="P53 path">
                <a id="p53-trace-archive" href="../index.html">ARCHIVE</a><span class="trace-separator">→</span>
            ...
            <a class="p53-index" href="index.html">ARCHIVE →</a>
        </header>
    ...
    <script>
        ...
        const archiveHref = `../index.html${{archiveParams.size ? `?${{archiveParams}}` : ""}}`;
        document.querySelectorAll("#p53-archive-return, #p53-trace-archive").forEach(link => link.href = archiveHref);
        if (filter && Object.hasOwn(filterLabels, filter)) {{
        ...
        if (artistRoute === expectedArtistRoute) {{
            ...
            const archiveReturn = document.querySelector("#p53-archive-return");
            archiveReturn.href = artistHref;
            archiveReturn.textContent = `← ${{artistName}}`;
        }}
...
def build_p53_archive(history: list[dict], current_slug: str) -> None:
    ...
    def broadcast_row(item: dict, label: str) -> str:
        return f'''<a class="broadcast-row" href="{html.escape(item["slug"], quote = True)}.html" style="--accent:{item["accent"]}">
            {cover(item)}
            <div><span>{label}</span><h2>{html.escape(item["track"])}</h2><p>{html.escape(item["artist"])}</p><small>{html.escape(item["album"])}</small></div>
        </a>'''

    current_html = ""
    if current:
        current_html = f'''<a class="current-broadcast" href="{html.escape(current["slug"], quote = True)}.html" style="--accent:{current["accent"]}">
            <div class="current-art">{cover(current)}</div>
            <div class="current-copy"><span>CURRENT TRANSMISSION</span><h2>{html.escape(current["track"])}</h2><p>{html.escape(current["artist"])}</p><small>{html.escape(current["album"])}</small><b>OPEN TRANSMISSION ↗</b></div>
        </a>'''
    recent_html = "".join(broadcast_row(item, "RETAINED SIGNAL") for item in recent)
    earlier_html = "".join(broadcast_row(item, "EARLIER TRANSMISSION") for item in earlier)
    earlier_section = f'''<details class="broadcast-section earlier"><summary><span>EARLIER TRANSMISSIONS</span><p>Retained without a forced order.</p><b>OPEN +</b></summary><div class="broadcast-list">{earlier_html}</div></details>''' if earlier_html else ""
    page = f'''<!DOCTYPE html>
...
.current-broadcast{{display:grid;grid-template-columns:minmax(260px,.85fr) minmax(0,1.15fr);min-height:440px;overflow:hidden;color:#fff;text-decoration:none;border:2px solid var(--accent);border-radius:20px 58px 20px 58px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent),transparent 76%),rgba(12,11,15,.95));box-shadow:10px 10px 0 color-mix(in srgb,var(--accent),transparent 70%),0 36px 75px rgba(0,0,0,.35);transition:transform .32s ease,border-radius .4s ease}} .current-broadcast:hover,.current-broadcast:focus-visible{{transform:translateY(-5px);border-radius:58px 20px 58px 20px}} .current-art{{min-height:100%;background:rgba(0,0,0,.25)}} .current-art img,.current-art .cover-missing{{display:block;width:100%;height:100%;min-height:440px;object-fit:cover}} .cover-missing{{display:grid;place-items:center;background:#17131b;color:#ff69ad;font-size:42px;font-weight:950}} .current-copy{{display:grid;align-content:end;padding:clamp(28px,5vw,68px)}} .current-copy h2{{max-width:720px;margin:12px 0 8px;font-size:clamp(46px,7vw,104px);line-height:.82;letter-spacing:-.055em}} .current-copy p{{margin:0;font-size:clamp(19px,2.5vw,30px)}} .current-copy small{{margin-top:7px;color:rgba(255,255,255,.6)}} .current-copy b{{margin-top:30px;color:color-mix(in srgb,var(--accent),white 38%);font-size:11px;letter-spacing:.16em}}
.broadcast-section{{margin-top:46px}} .section-label{{display:flex;justify-content:space-between;align-items:baseline;gap:18px;margin:0 0 12px;padding:0 4px}} .section-label p,.earlier summary p{{margin:0;color:rgba(255,255,255,.5);font-size:13px}} .broadcast-list{{display:grid;grid-template-columns:1fr;gap:10px}} .broadcast-row{{display:grid;grid-template-columns:118px 1fr;min-height:126px;overflow:hidden;color:#fff;text-decoration:none;border:1px solid color-mix(in srgb,var(--accent),white 22%);border-radius:22px 7px 22px 7px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent),transparent 82%),rgba(12,11,15,.94));transition:transform .3s ease,border-radius .3s ease}} .broadcast-row:hover,.broadcast-row:focus-visible{{transform:translateY(-4px);border-radius:7px 22px 7px 22px}} .broadcast-row img,.broadcast-row .cover-missing{{width:118px;height:126px;object-fit:cover}} .broadcast-row>div{{display:flex;flex-direction:column;justify-content:center;min-width:0;padding:16px}} .broadcast-row h2{{margin:7px 0 3px;font-size:clamp(21px,2vw,30px);line-height:.94}} .broadcast-row p{{margin:0 0 4px}} .broadcast-row small{{color:rgba(255,255,255,.54);font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}} .earlier{{padding:0;border:1px solid rgba(255,255,255,.13);border-radius:22px 7px 22px 7px;background:rgba(12,11,15,.7)}} .earlier summary{{display:grid;grid-template-columns:1fr auto;align-items:center;gap:3px 18px;padding:18px;cursor:pointer;list-style:none}} .earlier summary::-webkit-details-marker{{display:none}} .earlier summary span{{color:#ff7fbb;font-size:10px;font-weight:950;letter-spacing:.2em}} .earlier summary p{{grid-column:1}} .earlier summary b{{grid-column:2;grid-row:1/3;color:#ff8bc2;font-size:10px;letter-spacing:.12em}} .earlier[open] summary b{{font-size:0}} .earlier[open] summary b::after{{content:"CLOSE −";font-size:10px}} .earlier .broadcast-list{{padding:0 10px 10px}}
@media(max-width:760px){{main{{width:min(100% - 24px,620px);padding-top:12px}} nav span{{display:none}} .current-broadcast{{grid-template-columns:1fr;min-height:0;border-radius:15px 40px 15px 40px}} .current-art img,.current-art .cover-missing{{min-height:0;aspect-ratio:1}} .current-copy{{padding:28px 22px 32px}} .current-copy h2{{font-size:clamp(44px,14vw,76px)}} .broadcast-section{{margin-top:32px}} .section-label{{display:block}} .section-label p{{margin-top:6px}} .broadcast-row{{grid-template-columns:96px 1fr;min-height:116px}} .broadcast-row img,.broadcast-row .cover-missing{{width:96px;height:116px}}}}
@media(prefers-reduced-motion:reduce){{*,*::before,*::after{{animation-duration:.01ms!important;transition-duration:.01ms!important}}}}
</style></head><body><main><nav><a href="../index.html">← GSI</a><span>RADIO P53 / ARCHIVE</span><a href="latest.html">LATEST →</a></nav><header><span>EXTRACELLULAR SIGNALS / RETAINED</span><h1>RADIO P53</h1><p>Tracks I have or had on loop. The newest transmission leads; retained signals stay available without pretending to follow a schedule.</p></header>{current_html}<section class="broadcast-section"><div class="section-label"><span>RECENTLY RETAINED</span><p>The signals immediately before this one.</p></div><div class="broadcast-list">{recent_html}</div></section>{earlier_section}</main></body></html>'''
...
def build_artist_pages(tracks: list[dict], p53_history: list[dict]) -> None:
    ...
    artist_notes = load_config().get("artist_notes", {})
    for artist, artist_tracks in artist_groups.items():
        ...
        def entry_tile(item: dict, compact: bool = False) -> str:
            if compact:
                ...
                return f'''<a class="album-entry" href="{entry_href(item)}" style="--accent:{item["accent"]}">
                    <span class="album-entry-mark" aria-hidden="true"></span><h3>{html.escape(item["track"])}</h3><b>OPEN ↗</b>
                </a>'''
            return f'''<a class="artist-signal" href="{entry_href(item)}" style="--accent:{item["accent"]}">
        ...
        albums_html = "".join(
            f'''<section class="album-stack" style="--accent:{items[0]["accent"]}">
                <header><img src="../covers/{html.escape(items[0]["cover_file"], quote=True)}" alt="{html.escape(album, quote=True)} cover"><h2>{html.escape(album)}</h2></header>
...
</style></head><body><main><nav><a href="../index.html">ARCHIVE</a><span>→</span><strong>{html.escape(artist).upper()}</strong></nav><header class="artist-heading"><span>ARTIST SIGNALS / {len(artist_tracks):02d}</span><h1>{html.escape(artist)}</h1>{note_html}</header>{albums_html}<section class="artist-signals">{remaining_html}</section></main></body></html>'''
...
def build_index_html(tracks: list[dict]) -> None:  #sample homepage
    ...
    if p53_item:
        p53_html = f"""
        <a class="p53-broadcast" data-base-href="p53/latest.html" href="p53/latest.html" aria-label="Open the current P53 signal: {html.escape(p53_item['track'], quote = True)} by {html.escape(p53_item['artist'], quote = True)}" style="--signal-accent:{p53_item['accent']}">
            <div class="p53-art">
...
<head>
    ...
    <style>
        ...
        @media (max-width:760px) and (prefers-reduced-motion:no-preference) {{
            /* Phone-friendly motion: only transform and opacity, never an animated image filter. */
            .filter-description[data-filter="bassline"] .filter-room-label {{ animation:bass-pressure-mobile 1.6s ease-in-out infinite; }}
            .filter-description[data-filter="distortion"] .filter-decor {{ animation:distortion-scan-mobile 1.9s steps(4,end) infinite; }}
            @keyframes bass-pressure-mobile {{ 0%,100% {{ transform:skew(-8deg) scale(1); }} 42% {{ transform:skew(-8deg) scale(1.045,.95); }} 68% {{ transform:skew(-8deg) scale(.985,1.02); }} }}
            @keyframes distortion-scan-mobile {{ 0%,100% {{ transform:translateY(-5px); opacity:.34; }} 48% {{ transform:translateY(6px); opacity:.68; }} }}
        }}
    ...
    document.addEventListener("DOMContentLoaded", () => {{ // wait until the page exists before selecting buttons/cards
        ...
        function syncArchiveContext() {{
            document.querySelectorAll("[data-base-href]").forEach(link => {{
                link.href = archiveContextHref(link.dataset.baseHref);
            }});
            const params = new URLSearchParams();
            if (activeFilter) params.set("filter", activeFilter);
            const activeView = document.body.dataset.view;
