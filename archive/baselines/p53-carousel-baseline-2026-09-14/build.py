...
def build_p53_archive(history: list[dict], current_slug: str) -> None:
    ...
    def broadcast_row(item: dict) -> str:
        return f'''<a class="broadcast-row broadcast-record" data-base-href="{html.escape(item["slug"], quote = True)}.html" href="{html.escape(item["slug"], quote = True)}.html" style="--accent:{item["accent"]}">
            {cover(item)}
            <div><h2>{html.escape(item["track"])}</h2><p>{html.escape(item["artist"])}</p><small>{html.escape(item["album"])}</small></div>
        </a>'''

    current_html = ""
    if current:
        current_html = f'''<a class="current-broadcast broadcast-record is-active" data-base-href="{html.escape(current["slug"], quote = True)}.html" href="{html.escape(current["slug"], quote = True)}.html" style="--accent:{current["accent"]}">
            <div class="current-art">{cover(current)}</div>
            <div class="current-copy"><span>CURRENT TRANSMISSION</span><h2>{html.escape(current["track"])}</h2><p>{html.escape(current["artist"])}</p><small>{html.escape(current["album"])}</small><b>ENTER TRANSMISSION ↗</b></div>
        </a>'''
    history_html = "".join(broadcast_row(item) for item in remaining)
    page = f'''<!DOCTYPE html>
...
.current-broadcast{{display:grid;grid-template-columns:minmax(260px,.85fr) minmax(0,1.15fr);min-height:440px;overflow:hidden;color:#fff;text-decoration:none;border:2px solid var(--accent);border-radius:20px 58px 20px 58px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent),transparent 76%),rgba(12,11,15,.95));box-shadow:10px 10px 0 color-mix(in srgb,var(--accent),transparent 70%),0 36px 75px rgba(0,0,0,.35)}} .current-art{{min-height:100%;background:rgba(0,0,0,.25)}} .current-art img,.current-art .cover-missing{{display:block;width:100%;height:100%;min-height:440px;object-fit:cover}} .cover-missing{{display:grid;place-items:center;background:#17131b;color:#ff69ad;font-size:42px;font-weight:950}} .current-copy{{display:grid;align-content:end;padding:clamp(28px,5vw,68px)}} .current-copy>span{{display:inline-block;color:#ff7fbb;font-size:11px;font-weight:950;letter-spacing:.22em;transition:transform .45s ease,opacity .45s ease}} .current-copy h2{{max-width:720px;margin:12px 0 8px;font-size:clamp(46px,7vw,104px);line-height:.82;letter-spacing:-.055em}} .current-copy p{{margin:0;font-size:clamp(19px,2.5vw,30px)}} .current-copy small{{margin-top:7px;color:rgba(255,255,255,.6)}} .current-copy b{{margin-top:30px;color:color-mix(in srgb,var(--accent),white 38%);font-size:11px;letter-spacing:.16em}} .broadcast-record{{scroll-snap-align:start;scroll-snap-stop:always;transform:scale(.94);transform-origin:center top;transition:transform .48s cubic-bezier(.2,.8,.2,1),filter .36s ease;will-change:transform}} .broadcast-record.is-active{{transform:scale(1);filter:none}} .current-broadcast:not(.is-active) .current-copy>span{{opacity:.42;transform:scale(.82)}}
.broadcast-section{{margin-top:34px}} .broadcast-list{{display:grid;grid-template-columns:1fr;gap:18px}} .broadcast-row{{display:grid;grid-template-columns:clamp(150px,22vw,260px) 1fr;min-height:220px;overflow:hidden;color:#fff;text-decoration:none;border:1px solid color-mix(in srgb,var(--accent),white 22%);border-radius:22px 7px 22px 7px;background:linear-gradient(135deg,color-mix(in srgb,var(--accent),transparent 82%),rgba(12,11,15,.94));box-shadow:0 20px 45px rgba(0,0,0,.2)}} .broadcast-row img,.broadcast-row .cover-missing{{width:100%;height:100%;min-height:220px;object-fit:cover}} .broadcast-row>div{{display:flex;flex-direction:column;justify-content:center;min-width:0;padding:clamp(22px,4vw,46px)}} .broadcast-row h2{{margin:7px 0 5px;font-size:clamp(27px,3.3vw,54px);line-height:.9;letter-spacing:-.035em}} .broadcast-row p{{margin:0 0 7px;font-size:clamp(16px,1.8vw,24px)}} .broadcast-row small{{color:rgba(255,255,255,.54);font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}
@media(max-width:760px){{html{{scroll-snap-type:y proximity}} main{{width:min(100% - 24px,620px);padding-top:12px}} nav span{{display:none}} .current-broadcast,.broadcast-row{{grid-template-columns:1fr;min-height:calc(100svh - 24px);border-radius:15px 40px 15px 40px}} .current-art img,.current-art .cover-missing,.broadcast-row img,.broadcast-row .cover-missing{{min-height:0;height:auto;aspect-ratio:1;object-fit:cover}} .current-copy,.broadcast-row>div{{min-height:0;padding:28px 22px 32px}} .current-copy h2{{font-size:clamp(44px,14vw,76px)}} .broadcast-row h2{{font-size:clamp(38px,11vw,62px)}} .broadcast-section{{margin-top:22px}} .broadcast-list{{gap:22px}} .broadcast-record{{transform:scale(.92)}}}}
@media(prefers-reduced-motion:reduce){{*,*::before,*::after{{animation-duration:.01ms!important;transition-duration:.01ms!important}}}}
</style></head><body><main><nav><a id="p53-home-return" href="../index.html">GSI</a><span>→ RADIO P53</span></nav><header><h1>RADIO P53</h1><p>The ever-changing list of songs that hold a current value, scroll down to travel back in time.</p></header>{current_html}<section class="broadcast-section"><div class="broadcast-list">{history_html}</div></section></main><script>
...
function focusNearestTransmission() {{
    broadcastFrame = 0;
    const stage = window.innerHeight * .45;
    const nearest = broadcastRecords.reduce((closest, record) => {{
        const rect = record.getBoundingClientRect();
        const distance = Math.abs((rect.top + rect.height / 2) - stage);
        return !closest || distance < closest.distance ? {{ record, distance }} : closest;
    }}, null);
    if (!nearest) return;
    broadcastRecords.forEach(record => record.classList.toggle("is-active", record === nearest.record));
}}
window.addEventListener("scroll", () => {{
    if (!broadcastFrame) broadcastFrame = requestAnimationFrame(focusNearestTransmission);
}}, {{ passive:true }});
window.addEventListener("resize", focusNearestTransmission);
focusNearestTransmission();
</script></body></html>'''
...
</style></head><body><main><nav><a id="artist-archive-return" href="../index.html">GSI</a><span>→</span><strong>{html.escape(artist).upper()}</strong></nav><header class="artist-heading"><span>ARTIST SIGNALS / {len(artist_tracks):02d}</span><h1>{html.escape(artist)}</h1>{note_html}</header>{albums_html}<section class="artist-signals">{remaining_html}</section></main><script>
...
<head>
    ...
    <style>
        ...
        .filter-album-group {{
            ...
            background: linear-gradient(135deg, color-mix(in srgb, var(--accent), transparent 83%), rgba(18,18,20,.96));
            box-shadow: 0 18px 42px rgba(0,0,0,.25);
        }}
        .filter-album-cover {{ background: rgba(0,0,0,.25); }}
        .filter-album-cover img {{ display:block; width:100%; height:100%; object-fit:cover; }}
        .filter-album-copy {{ display:flex; flex-direction:column; justify-content:center; gap:9px; min-width:0; padding:22px; }}
        .filter-album-kicker {{ color:color-mix(in srgb, var(--accent), white 30%); font-size:10px; font-weight:900; letter-spacing:.16em; }}
        .filter-album-copy h2 {{ margin:0; font-size:clamp(25px,3vw,45px); line-height:.9; letter-spacing:-.04em; }}
        .filter-album-copy p {{ margin:0; color:rgba(255,255,255,.62); }}
        .filter-album-tracks {{ display:grid; gap:6px; margin-top:5px; }}
        .filter-album-track {{ display:flex; align-items:center; min-height:30px; color:#fff; text-decoration:none; font-size:13px; font-weight:800; }}
        .filter-album-track::before {{ content:""; width:7px; height:7px; flex:0 0 auto; margin-right:8px; border-radius:50%; background:var(--accent); box-shadow:0 0 10px var(--accent); }}
        .filter-album-track:hover, .filter-album-track:focus-visible {{ color:color-mix(in srgb, var(--accent), white 34%); }}
        body[data-view="poster"] .filter-album-group {{ grid-column:span 2; min-height:320px; }}
        body[data-view="gallery"] .filter-album-group {{ grid-template-columns:1fr; min-height:0; border-radius:10px; }}
        body[data-view="gallery"] .filter-album-copy {{ display:none; }}
        body[data-view="gallery"] .filter-album-cover img {{ aspect-ratio:1; }}
...
<script>
    document.addEventListener("DOMContentLoaded", () => {{ // wait until the page exists before selecting buttons/cards
        ...
        function renderFilterAlbumGroups(filterName) {{
            // A repeated record becomes one album pocket only inside the active filter.
            ...
            byAlbum.forEach(groupCards => {{
                ...
                const coverHtml = cover
                    ? `<div class="filter-album-cover"><img src="${{cover.getAttribute("src")}}" alt="${{first.dataset.album}} cover"></div>`
                    : '<div class="filter-album-cover" aria-hidden="true"></div>';
                const tracksHtml = groupCards.map(card => {{
                    const name = card.querySelector("h2").textContent;
                    return `<a class="filter-album-track" data-group-base-href="${{card.dataset.baseHref}}" href="${{archiveContextHref(card.dataset.baseHref)}}">${{name}}</a>`;
                }}).join("");
                group.innerHTML = `${{coverHtml}}<div class="filter-album-copy"><span class="filter-album-kicker">ALBUM / ${{String(groupCards.length).padStart(2, "0")}} SIGNALS</span><h2>${{first.dataset.album}}</h2><p>${{first.dataset.artist}}</p><div class="filter-album-tracks">${{tracksHtml}}</div></div>`;
                groupCards.forEach(card => card.style.display = "none");
