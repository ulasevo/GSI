...
<body>
    <main class="p53-page">
        <header class="p53-nav">
            ...
            <a class="p53-index" id="p53-index-link" href="index.html">ARCHIVE →</a>
        </header>
    ...
    <script>
        ...
        const p53IndexLink = document.querySelector("#p53-index-link");
        if (p53IndexLink) {{
...
def build_p53_archive(history: list[dict], current_slug: str) -> None:
    ...
    def broadcast_row(item: dict, label: str) -> str:
        return f'''<a class="broadcast-row" data-base-href="{html.escape(item["slug"], quote = True)}.html" href="{html.escape(item["slug"], quote = True)}.html" style="--accent:{item["accent"]}">
            {cover(item)}
    ...
    current_html = ""
    if current:
        current_html = f'''<a class="current-broadcast" data-base-href="{html.escape(current["slug"], quote = True)}.html" href="{html.escape(current["slug"], quote = True)}.html" style="--accent:{current["accent"]}">
...
</style></head><body><main><nav><a id="p53-home-return" href="../index.html">← GSI</a><span>RADIO P53 / ARCHIVE</span><a id="p53-latest-link" href="latest.html">LATEST →</a></nav><header><span>EXTRACELLULAR SIGNALS / RETAINED</span><h1>RADIO P53</h1><p>Tracks I have or had on loop. The newest transmission leads; retained signals stay available without pretending to follow a schedule.</p></header>{current_html}<section class="broadcast-section"><div class="section-label"><span>RECENTLY RETAINED</span><p>The signals immediately before this one.</p></div><div class="broadcast-list">{recent_html}</div></section>{earlier_section}</main><script>
...
document.querySelector("#p53-latest-link").href = `latest.html${{p53ArchiveParams.size ? `?${{p53ArchiveParams}}` : ""}}`;
</script></body></html>'''
...
def build_artist_pages(tracks: list[dict], p53_history: list[dict]) -> None:
    ...
    artist_filter_keys_json = json.dumps(list(load_config().get("filters", {}).keys()))
...
</style></head><body><main><nav><a id="artist-archive-return" href="../index.html">ARCHIVE</a><span>→</span><strong>{html.escape(artist).upper()}</strong></nav><header class="artist-heading"><span>ARTIST SIGNALS / {len(artist_tracks):02d}</span><h1>{html.escape(artist)}</h1>{note_html}</header>{albums_html}<section class="artist-signals">{remaining_html}</section></main><script>
...
document.querySelector("#artist-archive-return").href = `../index.html${{artistRoomParams.size ? `?${{artistRoomParams}}` : ""}}`;
