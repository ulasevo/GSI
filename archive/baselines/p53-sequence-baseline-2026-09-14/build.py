...
const broadcastObserver = new IntersectionObserver(entries => {{
    const visible = entries.filter(entry => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
    if (!visible) return;
    broadcastRecords.forEach(record => record.classList.toggle("is-active", record === visible.target));
}}, {{ threshold:[.35,.55,.75], rootMargin:"-12% 0px -30% 0px" }});
broadcastRecords.forEach(record => broadcastObserver.observe(record));
</script></body></html>'''
