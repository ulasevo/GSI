...
def build_index_html(tracks: list[dict]) -> None:  #sample homepage
    ...
    if intro.strip():
        ...
        """
    filter_buttons = ['<button class="filter-btn active" data-filter="all">All</button>'] # All button always exists

    filter_data = { # JavaScript will use this to update the description panel
        "all": {
            "label": "All",
            "description": "Every active inducer in the cabinet.",
            "color": "#ffffff"
        }
    }
    ...
    <section class="filter-panel">
        ...
        <div class="filter-description">
            <h2 id="filter-title">All</h2>
            <p id="filter-description">Every active inducer in the cabinet.</p>
        </div>
    </section>
    """
...
<head>
    ...
    <style>
        ...
        .intro p {{
            ...
            padding: 18px;
            border-radius: 24px;
            background: color-mix(in srgb, var(--page-tint, #ffffff), #1b1b1b 88%);
            border: 1px solid color-mix(in srgb, var(--page-tint, #ffffff), white 12%);
            box-shadow: 0 0 35px color-mix(in srgb, var(--page-tint, #ffffff), transparent 82%);
...
<script>
    document.addEventListener("DOMContentLoaded", () => {{ // wait until the page exists before selecting buttons/cards
        ...
        function setFilter(filterName) {{ // changes active filter and visible cards
            const info = filterInfo[filterName] || filterInfo["all"]; // fallback to All if key is missing

            document.documentElement.style.setProperty("--page-tint", info.color); // update glow/tint color
            title.textContent = info.label; // update panel title
            description.textContent = info.description; // update panel description

            buttons.forEach(button => {{ // update which button looks active
                button.classList.toggle("active", button.dataset.filter === filterName);
            }});

            cards.forEach(card => {{ // show or hide each card
                const tags = card.dataset.tags || ""; // read tags from data-tags
                const shouldShow = filterName === "all" || tags.includes(filterName); // all shows everything
                card.style.display = shouldShow ? "block" : "none"; // apply visibility
            }});
        }}

        buttons.forEach(button => {{ // make each filter button clickable
            button.addEventListener("click", () => {{
                setFilter(button.dataset.filter);
            }});
        }});

        setFilter("all"); // initialize page in All mode
    }});
</script>
</body>
</html>
