...
def build_index_html(tracks: list[dict]) -> None:  #sample homepage
    ...
    if p53_item:
        p53_html = f"""
        <a class="p53-broadcast" href="p53/latest.html" aria-label="Open the current P53 signal: {html.escape(p53_item['track'], quote = True)} by {html.escape(p53_item['artist'], quote = True)}" style="--signal-accent:{p53_item['accent']}">
            <div class="p53-art">
