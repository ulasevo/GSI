...
def download_cover(url: str, save_path: Path) -> None:
    response = requests.get(url, timeout = 15)
    response.raise_for_status()
    save_path.write_bytes(response.content) # save to disk
...
def build_entries() -> list[dict]: # Main Builder for markdown and covers
    ...
    for row in read_tracks(): #loop songs in tracks.csv
        ...
        if not cover_path.exists(): #so we don't overwrite
